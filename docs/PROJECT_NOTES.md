...
2025년 수업 프로젝트는 교수자가 지정한 KISA 「주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드」 2021.3판을 기준으로 구현했다. 기존 파일명과 항목 번호는 옛 번호를 잘못 사용한 것이 아니라 2021.3 가이드의 실제 번호를 보존한 것이다.
...
KISA가 2025-12-24 게시한 최신 상세가이드(통상 2026 가이드)는 항목 번호와 구조가 바뀌었다. 아래 표는 원 프로젝트의 기준을 바꾸거나 기존 번호를 정정하는 것이 아니라, 2026년 포트폴리오 공개 시점에 확인한 의미상 대응 관계다.
...
- fixture 기반 U-52 clean/PASS·duplicate/FAIL·`MANUAL_REQUIRED` 검증
- fixture 기반 U-57 정상/PASS·취약/FAIL·`fixed: false` 검증
- fixture 기반 W-50 한국어·영어·파싱 오류 `net accounts` 입력 검증

PSScriptAnalyzer finding은 CI 로그에 모두 표시한다. Error-level finding만 실패 조건으로 삼으며, 대화형 관리 스크립트의 상태·경고 출력에 사용하는 `Write-Host` 관련 Warning은 현재 허용한다. Warning을 숨기거나 suppress하지 않는다.
